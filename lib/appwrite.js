...
export const searchMeasurements = async (query) => {
  try {
    const measurements = await databases.listDocuments(databaseId, measurementId, [Query.search("timestamp", query)]);
...
export const getLatestPowerEnergyData = async () => {
  try {
    ...
    const response = await databases.listDocuments(appwConfig.databaseId, appwConfig.measurementId, [
    ...
    const labels = documents.map((doc) => new Date(doc.$createdAt).toLocaleTimeString()); // Convert timestamps to readable time
